...
A Godot 4 template designed for building online and local multiplayer games.

## ActionRouter

The `ActionRouter` ([action_router.gd](./scripts/input/action_router.gd)) is a [Node](https://docs.godotengine.org/en/stable/classes/class_node.html) that clones [InputMap](https://docs.godotengine.org/en/stable/classes/class_inputmap.html) actions to isolate inputs per device. This allows multiple players (Keyboard/Mouse or Gamepads) to use the same action names without interference.

### Usage

1. **Inherit**: Create a class (e.g., [GameActionRouter](./scripts/input/game_action_router.gd)) and list actions to monitor in `_init`.
